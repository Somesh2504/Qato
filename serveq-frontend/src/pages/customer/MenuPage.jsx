    ...
    <div className="min-h-screen bg-white pb-32">
      <div ref={headerRef} className="sticky top-0 z-20 bg-white shadow-sm">
        ...
        {isReconnecting ? (
          <div className="px-4 py-2">
            <p className="text-xs text-[#FF6B35] font-semibold">Reconnecting…</p>
          </div>
        ) : null}
      ...
      <div className="px-4 py-4 space-y-8">
        {loading ? (
          ...
          <div className="pt-8">
            <EmptyState
              icon="🍽️"
              title="Menu coming soon — check back shortly."
        ...
        <div className="fixed bottom-6 left-4 right-4 z-30 animate-bounce-in">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white/60 text-sm">Proceed to Pay →</span>
            </div>
          </button>
        </div>
      )}
      ...
      >
        <div className="p-4 space-y-4">
          {items.length === 0 ? (
            <EmptyState icon="🛒" title="Cart is empty" description="Add items from menu to continue." />
          ) : (
            <>
                    ...
                    >
                      <div className="flex-1 p-4 min-w-0">
                        <div className="flex items-center gap-1.5 mb-1.5">
                          ...
                          {item.is_bestseller && <Badge variant="warning" size="xs">⭐ Bestseller</Badge>}
                        </div>
                        <h3 className="font-semibold text-[#1A1A2E] text-sm leading-tight">{item.name}</h3>
                        {item.description && (
                          <p className="text-xs text-gray-400 mt-1 line-clamp-2 leading-relaxed">{item.description}</p>
    ...
    <div className="min-h-screen bg-white pb-32">
      ...
      <div className="px-4 py-4 space-y-8">
        {loading ? (
          ...
          <EmptyState icon="🔍" title="No items found" description={search ? 'Try a different search' : 'Menu is empty'} />
        ) : (
          Object.entries(grouped).map(([cat, catItems]) => (
            <div key={cat} ref={el => categoryRefs.current[cat] = el}>
              <h2 className="text-base font-bold text-[#1A1A2E] mb-3 flex items-center gap-2">
                    ...
                    <div key={item.id} className="bg-white rounded-[12px] border border-gray-100 shadow-sm flex gap-3 overflow-hidden hover:shadow-md transition-shadow animate-fade-in">
                      ...
                      <div className="flex-1 p-4 min-w-0">
                        <div className="flex items-center gap-1.5 mb-1.5">
                          ...
                          {item.is_bestseller && (
                            <Badge variant="warning" size="xs">⭐ Bestseller</Badge>
                          )}
                        </div>
                        <h3 className="font-semibold text-[#1A1A2E] text-sm leading-tight">{item.name}</h3>
                        {item.description && (
